...
class ClassForm extends ClassUsers{
    ...
    public function users_novo(){
        ...
        if($users->execute()){
                ...
                $mail->SMTPAuth = $Config_Mail->config_mail_smtp_auth == 1 ? true : false;      // Enable SMTP authentication
                ...
                $mail->Subject = 'Acesso ao Portal '.SITE_TITLE;
                
                $message = '<table border="0" width="100%">'
                        . '<tr>'
                            . '<td>'
                                . '<br/>'
                                . '<center><img src="'.SITE.'img/logo_color.png" width="190" /></center>'
                            . '</td>'
                        . '</tr>'
                        . '<tr>'
                            . '<td align="center">'
    ...
    public static function users_convite_busca($email, $limit = 3, $fetch = false, $like = true){
    ...
    public function users_convite_delete(){
        $sql = "DELETE FROM convites WHERE convite_email = :email";
        $users = ClassDataBase::prepare($sql);
        $users->bindValue(':email', $this->form_email);
        if($users->execute())
            return true;
        else
            return false;
